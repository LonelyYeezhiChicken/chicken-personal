...
Broker 端設定（`/conf/activemq.xml`）：

```xml
<policyEntry queue=">" queuePrefetch="100"/>
```

客戶端也可透過 `activemq.prefetchPolicy.queuePrefetch` 覆寫。詳見 [`destinationPolicy`](/docs/activeMQ/advanced/destinationPolicy)。
...
```java
producer.setTimeToLive(60 * 1000L); // 60 秒後過期
// 或針對單則訊息
producer.send(message, DeliveryMode.PERSISTENT, 4, 60 * 1000L);
```

過期訊息可透過 DLQ 的 `processExpired="true"` 保留，參見 [`deadLetterQueue`](/docs/activeMQ/usage/deadLetterQueue)。
...
完整說明參見 [`ackAndRedelivery`](/docs/activeMQ/usage/ackAndRedelivery)。
